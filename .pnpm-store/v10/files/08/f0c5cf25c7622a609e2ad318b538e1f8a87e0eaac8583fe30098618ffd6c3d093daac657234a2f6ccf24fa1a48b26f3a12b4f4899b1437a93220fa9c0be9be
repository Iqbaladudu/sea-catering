...
const buildTableState = async args => {
  ...
  const listPreferences = await upsertPreferences({
    key: Array.isArray(collectionSlug) ? `${parent.collectionSlug}-${parent.joinPath}` : `${collectionSlug}-list`,
    req,
    value: {
      columns,
  ...
  const {
    ...
    columnPreferences: Array.isArray(collectionSlug) ? listPreferences?.columns : undefined,
    columns,
    docs,
    enableRowSelections,
    i18n: req.i18n,
